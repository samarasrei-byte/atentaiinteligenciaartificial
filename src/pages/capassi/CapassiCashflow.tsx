...
import {
  ...
  ReferenceLine, Legend, AreaChart, Area, ComposedChart, Bar, Line
} from 'recharts';
...
export default function CapassiCashflow() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      let query = supabase.from('capassi_transactions' as any).select('amount_cents, date, type').eq('organization_id', currentOrg.id);
      ...
      setData(
        Object.entries(byMonth).sort(([a], [b]) => a.localeCompare(b)).map(([month, vals]) => {
          const net = vals.revenue - vals.costs;
          runningBalance += net;
          return { month: month.substring(5), revenue: vals.revenue, costs: vals.costs, net, balance: runningBalance };
        })
      );
    };
    fetch();
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-white/30">Projeção de entradas, saídas e saldo — {currentOrg.name}</p>
      </div>
      ...
      <Card className="bg-white/[0.02] border-white/[0.06]">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm text-white/80 flex items-center gap-2">
            <TrendingUp className="h-4 w-4 text-[#55FFAA]" /> Fluxo Mensal com Saldo Acumulado
          </CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card className="bg-white/[0.02] border-white/[0.06]">
        <CardHeader className="pb-2"><CardTitle className="text-sm text-white/80">Resultado Líquido Mensal</CardTitle></CardHeader>
        <CardContent>
