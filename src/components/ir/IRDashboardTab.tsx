...
import {
  ...
  Loader2, Eye, FileUp, Trash2, BarChart3, Zap,
  Star, Lock, Download
} from 'lucide-react';
...
export default function IRDashboardTab() {
  ...
  React.useEffect(() => {
    ...
    const channel = supabase
      .channel('user-ir-realtime-tab')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'ir_ai_declarations', filter: `user_id=eq.${user.id}` }, () => loadDeclarations())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'ir_ai_documents', filter: `user_id=eq.${user.id}` }, () => {
  ...
  const goToIRPayment = () => navigate('/ir');
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(e.target.files)) {
      ...
      if (!allowedTypes.includes(file.type)) { toast.error(`"${file.name}" formato não suportado.`); continue; }

      const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_').slice(0, 100);
      const filePath = `${user.id}/${activeDeclaration.id}/${Date.now()}_${safeName}`;

      const { error: uploadError } = await supabase.storage.from('ir-ai-documents').upload(filePath, file);
    ...
    if (uploadedCount > 0) toast.success(`${uploadedCount} documento(s) enviado(s)!`);
  ...
  const analyzeDocument = async (doc: DocFile) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('ai-ir-analyze', {
        body: { declarationId: activeDeclaration?.id, documentId: doc.id, documentType: doc.document_type, action: 'analyze_document' },
      });
      if (error) throw error;
      if (data?.error) { toast.error(data.error); if (activeDeclaration) loadDocuments(activeDeclaration.id); setAnalyzingDocId(null); return; }
  ...
  const handleDownloadDocument = async (doc: DocFile) => {
    try {
      const { data, error } = await supabase.storage.from('ir-ai-documents').createSignedUrl(doc.file_path, 300);
  ...
  const handleDownloadPDF = async () => {
    ...
    if (analysis.model_comparison) {
      ...
      if (simp) {
        pdf.text('SIMPLIFICADO', 20, y); y += 5;
    ...
    if (analysis.deduction_items?.length > 0) {
      ...
      pdf.text('Deduções Identificadas', 15, y); y += 8;
    ...
    if (analysis.income_sources?.length > 0) {
      if (y > 240) { pdf.addPage(); y = 15; }
      pdf.setTextColor(30, 41, 59); pdf.setFontSize(13); pdf.setFont('helvetica', 'bold');
      pdf.text('Fontes de Rendimento', 15, y); y += 8;
      pdf.setFontSize(9); pdf.setFont('helvetica', 'normal'); pdf.setTextColor(71, 85, 105);
    ...
    if (analysis.malha_fina_risk) {
      ...
      pdf.setTextColor(30, 41, 59); pdf.setFontSize(13); pdf.setFont('helvetica', 'bold');
      pdf.text(`Risco de Malha Fina: ${analysis.malha_fina_risk}`, 15, y); y += 8;
      pdf.setFontSize(9); pdf.setFont('helvetica', 'normal'); pdf.setTextColor(71, 85, 105);
    ...
    const disc = 'Esta análise é gerada por inteligência artificial e tem caráter informativo. A responsabilidade pela declaração é do contribuinte.';
    pdf.text(pdf.splitTextToSize(disc, pw - 30), 15, y);
  ...
  const handleChecklistComplete = async (answers: ChecklistAnswers) => {
    ...
    if (saveError) { toast.error('Erro ao salvar checklist.'); return; }
  ...
  const handleSkipChecklist = async () => {
    ...
    setChecklistCompleted(true);
    if (activeDeclaration) {
      await supabase.from('ir_ai_declarations').update({ checklist_completed: true } as any).eq('id', activeDeclaration.id);
    }
  ...
  const generateSummary = async (checklistData?: ChecklistAnswers) => {
    ...
    try {
      const { error } = await supabase.functions.invoke('ai-ir-analyze', {
        body: { declarationId: activeDeclaration.id, action: 'generate_summary', checklistData: checklistData || null },
      });
      if (error) throw error;
  ...
  const getProgress = (status: string) => {
    const map: Record<string, number> = { draft: 10, pending_documents: 25, processing: 50, ai_analysis: 75, review: 90, completed: 100, error: 0 };
    return map[status] ?? 5;
  };
    ...
    <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {declarations.map((decl) => {
              ...
              return (
                <Button key={decl.id} size="sm" variant={isActive ? 'default' : 'outline'}
                  onClick={() => { activeDeclarationRef.current = decl.id; setActiveDeclaration(decl); loadDocuments(decl.id); }}
      ...
      {activeDeclaration && (
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-muted/50 border border-border rounded-full p-1 mb-8">
            <TabsTrigger value="overview" className="rounded-full data-[state=active]:bg-purple-500 data-[state=active]:text-white text-muted-foreground text-sm px-5">Visão geral</TabsTrigger>
            <TabsTrigger value="documents" className="rounded-full data-[state=active]:bg-purple-500 data-[state=active]:text-white text-muted-foreground text-sm px-5">Documentos ({documents.length})</TabsTrigger>
            <TabsTrigger value="analysis" className="rounded-full data-[state=active]:bg-purple-500 data-[state=active]:text-white text-muted-foreground text-sm px-5">Análise IA</TabsTrigger>
            <TabsTrigger value="result" className="rounded-full data-[state=active]:bg-purple-500 data-[state=active]:text-white text-muted-foreground text-sm px-5">Resultado</TabsTrigger>
          </TabsList>
                ...
                <Card className="bg-red-500/5 border-red-500/20">
                  <CardContent className="p-5 flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground mb-3">Tente reenviar os documentos.</p>
                ...
                <CardContent className="p-6">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        {activeDeclaration.ai_confidence_percent != null && activeDeclaration.ai_confidence_percent > 0 && (
                          <Badge className="bg-emerald-500/10 text-emerald-400 border-0">{activeDeclaration.ai_confidence_percent}% confiança</Badge>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="mt-4">
                    <div className="flex justify-between text-xs text-muted-foreground mb-2">
                      <span>Progresso</span><span>{getProgress(activeDeclaration.status)}%</span>
                    </div>
              ...
              {[
                ...
                >
                  <CardContent className="p-5">
                    ...
                    {s.active && !s.done && <p className="text-xs text-purple-400 mt-2 font-medium">← Próximo passo</p>}
                  </CardContent>
                </Card>
              ))}
            </div>
            ...
            <div className="space-y-6">
              <Card className="bg-card border-border border-dashed">
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-xs text-amber-400 mb-4">⚠️ Selecione o tipo correto abaixo antes de enviar cada documento</p>

                  <div className="flex flex-wrap items-center justify-center gap-2 mb-4">
                    {Object.entries(docTypeLabels).map(([key, label]) => (
                      <Button key={key} size="sm" variant={selectedDocType === key ? 'default' : 'outline'}
                  ...
                  <label className="cursor-pointer">
                    <input type="file" multiple accept=".pdf,.jpg,.jpeg,.png,.webp" className="hidden" onChange={handleUpload} disabled={isUploading} />
                  ...
                  <div className="mt-6 p-4 rounded-xl bg-muted/40 border border-border text-left max-w-lg mx-auto">
                    <p className="text-xs font-semibold text-foreground mb-2 flex items-center gap-1.5">
                      <AlertCircle className="w-3.5 h-3.5 text-amber-400" /> Dicas para melhor resultado
                    </p>
                    <ul className="space-y-1.5 text-xs text-muted-foreground">
                      <li className="flex items-start gap-2"><span className="text-amber-400 mt-0.5">•</span><span>PDFs escaneados com <strong className="text-foreground">baixa qualidade</strong> podem ter extração imprecisa.</span></li>
                      <li className="flex items-start gap-2"><span className="text-red-400 mt-0.5">•</span><span>Documentos <strong className="text-foreground">protegidos por senha</strong> não podem ser processados.</span></li>
                      <li className="flex items-start gap-2"><span className="text-purple-400 mt-0.5">•</span><span>A IA exibe um <strong className="text-foreground">índice de confiança</strong> para cada extração.</span></li>
                    </ul>
                  </div>
                </CardContent>
              </Card>
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      {pendingDocs.length > 0 && (
                        <Button size="sm" onClick={analyzeAllPending} disabled={isAnalyzing} className="bg-purple-500 hover:bg-purple-600 rounded-full text-xs">
                          {isAnalyzing ? <Loader2 className="w-3 h-3 mr-1 animate-spin" /> : <Brain className="w-3 h-3 mr-1" />}
                      ...
                      <div key={doc.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/30 border border-border">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <Badge className={
                            ...
                            {doc.ai_status === 'extracted' ? '✅ Extraído' : doc.ai_status === 'processing' ? '⏳ Processando' : doc.ai_status === 'error' ? '❌ Erro' : '⏸️ Pendente'}
                          </Badge>
          ...
          <TabsContent value="analysis">
            <div className="space-y-6">
              {extractedDocs.length === 0 ? (
                ...
                <>
                  {showChecklist ? (
                    <IRPreAnalysisChecklist onComplete={handleChecklistComplete} onSkip={handleSkipChecklist} isLoading={isAnalyzing} fiscalYear={activeDeclaration.fiscal_year} />
                  ) : (
                    <Card className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border-purple-500/20">
                      <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {checklistCompleted ? 'Checklist concluído! Gere o resumo completo.' : 'Responda o checklist fiscal para uma análise mais precisa'}
                            </p>
                          </div>
                          <Button onClick={() => { if (checklistCompleted) generateSummary(); else setShowChecklist(true); }} disabled={isAnalyzing} className="bg-purple-500 hover:bg-purple-600 rounded-full">
                    ...
                    <Card key={doc.id} className="bg-card border-border">
                      <CardHeader className="pb-3">
                        <CardTitle className="text-sm flex items-center gap-2">
                          ...
                          {doc.file_name}
                          <Badge className="bg-emerald-500/10 text-emerald-400 border-0 text-[10px]">{docTypeLabels[doc.document_type] || doc.document_type}</Badge>
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        {doc.ai_extracted_data && (
                          <div className="space-y-2 text-sm">
                            {doc.ai_extracted_data.source_name && <p className="text-muted-foreground"><span className="text-muted-foreground/60">Fonte:</span> {doc.ai_extracted_data.source_name}</p>}
                            {doc.ai_extracted_data.source_document && <p className="text-muted-foreground"><span className="text-muted-foreground/60">CNPJ/CPF:</span> {doc.ai_extracted_data.source_document}</p>}
                            ...
                            {doc.ai_extracted_data.observations && <p className="text-xs text-muted-foreground/70 mt-2 italic">📝 {doc.ai_extracted_data.observations}</p>}
                            {doc.ai_extracted_data.confidence_percent != null && (
                              <div className="flex items-center gap-2 mt-2">
                                ...
                                <span className="text-xs text-muted-foreground">{doc.ai_extracted_data.confidence_percent}% confiança</span>
                              </div>
                            )}
                          </div>
                        )}
            ...
            {!analysis ? (
              ...
              <div className="space-y-6">
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        <Button size="sm" onClick={handleDownloadPDF} className="bg-purple-500 hover:bg-purple-600 rounded-full text-xs">
                          <Download className="w-3 h-3 mr-1" /> Baixar PDF
                        </Button>
                        {activeDeclaration.ai_confidence_percent != null && (
                          <Badge className="bg-emerald-500/20 text-emerald-400 border-0">{activeDeclaration.ai_confidence_percent}% confiança</Badge>
                        )}
                      </div>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="p-4 rounded-xl bg-muted/30"><p className="text-xs text-muted-foreground">Rendimentos</p><p className="text-lg font-bold text-blue-400">{formatCurrency(activeDeclaration.total_income_cents)}</p></div>
                      <div className="p-4 rounded-xl bg-muted/30"><p className="text-xs text-muted-foreground">Deduções</p><p className="text-lg font-bold text-emerald-400">{formatCurrency(activeDeclaration.total_deductions_cents)}</p></div>
                      <div className="p-4 rounded-xl bg-muted/30"><p className="text-xs text-muted-foreground">Imposto</p><p className="text-lg font-bold text-amber-400">{formatCurrency(activeDeclaration.tax_due_cents)}</p></div>
                      <div className="p-4 rounded-xl bg-muted/30"><p className="text-xs text-muted-foreground">Restituição</p><p className="text-lg font-bold text-emerald-400">{formatCurrency(activeDeclaration.refund_cents)}</p></div>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <CardContent className="p-5">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          {analysis.recommendation_reason && <p className="text-sm text-muted-foreground mt-1">{analysis.recommendation_reason}</p>}
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className={`p-4 rounded-xl border ${analysis.recommended_model === 'simplificado' ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-border bg-muted/20'}`}>
                          <div className="flex items-center justify-between mb-3">
                            ...
                            {analysis.recommended_model === 'simplificado' && <Badge className="bg-emerald-500/20 text-emerald-400 border-0 text-[10px]">✓ Melhor</Badge>}
                          </div>
                          <div className="space-y-2 text-xs">
                            <div className="flex justify-between"><span className="text-muted-foreground">Deduções</span><span className="font-medium">{formatCurrency(analysis.model_comparison.simplified?.deductions_cents || 0)}</span></div>
                            <div className="flex justify-between"><span className="text-muted-foreground">Base de cálculo</span><span className="font-medium">{formatCurrency(analysis.model_comparison.simplified?.taxable_base_cents || 0)}</span></div>
                            <div className="flex justify-between pt-1 border-t border-border/50"><span className="text-muted-foreground font-medium">Imposto</span><span className="font-bold text-amber-400">{formatCurrency(analysis.model_comparison.simplified?.tax_cents || 0)}</span></div>
                          </div>
                        </div>
                        <div className={`p-4 rounded-xl border ${analysis.recommended_model === 'completo' ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-border bg-muted/20'}`}>
                          <div className="flex items-center justify-between mb-3">
                            ...
                            {analysis.recommended_model === 'completo' && <Badge className="bg-emerald-500/20 text-emerald-400 border-0 text-[10px]">✓ Melhor</Badge>}
                          </div>
                          <div className="space-y-2 text-xs">
                            <div className="flex justify-between"><span className="text-muted-foreground">Deduções</span><span className="font-medium">{formatCurrency(analysis.model_comparison.complete?.deductions_cents || 0)}</span></div>
                            <div className="flex justify-between"><span className="text-muted-foreground">Base de cálculo</span><span className="font-medium">{formatCurrency(analysis.model_comparison.complete?.taxable_base_cents || 0)}</span></div>
                            <div className="flex justify-between pt-1 border-t border-border/50"><span className="text-muted-foreground font-medium">Imposto</span><span className="font-bold text-amber-400">{formatCurrency(analysis.model_comparison.complete?.tax_cents || 0)}</span></div>
                          </div>
                        </div>
                      </div>
                      {(() => {
                ...
                {analysis.deduction_items?.length > 0 && (
                  <Card className="bg-card border-border">
                    <CardHeader className="pb-3">
                      <CardTitle className="text-sm flex items-center gap-2 text-emerald-400"><TrendingDown className="w-4 h-4" /> Deduções Identificadas</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-2">
                      {analysis.deduction_items.map((ded: any, i: number) => (
                        <div key={i} className="flex items-center justify-between p-2 rounded bg-muted/30">
                          <div><span className="text-sm">{ded.description}</span>{ded.category && <span className="text-xs text-muted-foreground ml-2">({ded.category})</span>}</div>
                ...
                {analysis.alerts?.length > 0 && (
                  <Card className="bg-card border-amber-500/20">
                    <CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2 text-amber-400"><AlertCircle className="w-4 h-4" /> Alertas</CardTitle></CardHeader>
                    <CardContent className="space-y-2">
                      {analysis.alerts.map((alert: string, i: number) => (
                        <div key={i} className="flex items-start gap-2 text-sm"><AlertCircle className="w-3 h-3 text-amber-400 mt-1 shrink-0" /><p className="text-muted-foreground">{alert}</p></div>
                      ))}
                    </CardContent>
                  </Card>
                )}
                ...
                {analysis.optimization_tips?.length > 0 && (
                  <Card className="bg-card border-emerald-500/20">
                    <CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2 text-emerald-400"><TrendingUp className="w-4 h-4" /> Dicas de Otimização</CardTitle></CardHeader>
                    <CardContent className="space-y-2">
                      {analysis.optimization_tips.map((tip: string, i: number) => (
                        <div key={i} className="flex items-start gap-2 text-sm"><CheckCircle2 className="w-3 h-3 text-emerald-400 mt-1 shrink-0" /><p className="text-muted-foreground">{tip}</p></div>
                      ))}
                    </CardContent>
                  </Card>
                )}
                ...
                {analysis.income_sources?.length > 0 && (
                  <Card className="bg-card border-border">
                    <CardHeader className="pb-3"><CardTitle className="text-sm">Fontes de Rendimento</CardTitle></CardHeader>
                    <CardContent className="space-y-2">
                      {analysis.income_sources.map((src: any, i: number) => (
                        <div key={i} className="flex items-center justify-between p-2 rounded bg-muted/30 gap-3">
                          ...
                          <div className="flex items-center gap-4 shrink-0">
                            {src.irrf_cents != null && src.irrf_cents > 0 && <span className="text-xs text-amber-400" title="IRRF retido na fonte">IRRF: {formatCurrency(src.irrf_cents)}</span>}
                ...
                {analysis.malha_fina_risk && (
                  <Card className={`border ${analysis.malha_fina_risk === 'alto' ? 'bg-red-500/5 border-red-500/20' : analysis.malha_fina_risk === 'medio' ? 'bg-amber-500/5 border-amber-500/20' : 'bg-emerald-500/5 border-emerald-500/20'}`}>
                    <CardContent className="p-5">
                      <div className="flex items-start gap-3">
                        <Shield className={`w-5 h-5 mt-0.5 ${analysis.malha_fina_risk === 'alto' ? 'text-red-400' : analysis.malha_fina_risk === 'medio' ? 'text-amber-400' : 'text-emerald-400'}`} />
                        <div>
                          <p className="font-medium">Risco de Malha Fina: <span className={`capitalize ${analysis.malha_fina_risk === 'alto' ? 'text-red-400' : analysis.malha_fina_risk === 'medio' ? 'text-amber-400' : 'text-emerald-400'}`}>{analysis.malha_fina_risk}</span></p>
                          {analysis.malha_fina_reasons?.length > 0 && (
                            <ul className="mt-2 space-y-1">
                              {analysis.malha_fina_reasons.map((reason: string, i: number) => (
                                <li key={i} className="text-sm text-muted-foreground flex items-start gap-2"><AlertCircle className="w-3 h-3 mt-1 shrink-0" />{reason}</li>
                              ))}
                            </ul>
                          )}
                        </div>
      ...
      {declarations.length > 0 && (
        <div className="flex justify-center pt-4">
          <Button onClick={goToIRPayment} variant="outline" className="rounded-full">
            <Sparkles className="w-4 h-4 mr-2" /> Nova Declaração de IR
          </Button>
        </div>
      )}
    </div>
